! function(a) {
	...
	b.dialog = function(a, b, c, d) {
		...
		if(c = $.extend({}, d, c), !c.orderFee || c.orderFee <= 0) return void b.toast("请确认支付金额", "", "error");
		!c.defaultMethod && c.payMethod && (c.defaultMethod = c.payMethod);
		var e = mui.className("active"),
			...
			k = function() {
				$.post("index.php?i=" + a.sysinfo.uniacid + "&j=" + a.sysinfo.acid + "&c=entry&m=core&do=pay", {
					method: c.payMethod,
					tid: c.orderTid,
					title: c.orderTitle,
					fee: c.orderFee,
					...
					if(b.loading().hide(), a = $.parseJSON(a), a.message.errno) {
						var d = {
							errno: a.message.errno,
							message: a.message.message
						};
						return c.fail(d), void c.complete(d)
					}
					payment = a.message.message, WeixinJSBridge.invoke("getBrandWCPayRequest", {
						appId: payment.appId,
						...
						if("get_brand_wcpay_request:ok" == a.err_msg) {
							...
							c.success(b), c.complete(b)
						} else if("get_brand_wcpay_request:cancel" == a.err_msg) {
							var b = {
								errno: -1,
								message: a.err_msg
							};
							...
							c.fail(b), c.complete(b)
						}
					})
				})
			},
			l = function() {
				b.loading().hide(), $.post("index.php?i=" + a.sysinfo.uniacid + "&j=" + a.sysinfo.acid + "&c=entry&m=core&do=pay", {
					method: c.payMethod,
					tid: c.orderTid,
					title: c.orderTitle,
					fee: c.orderFee,
			...
			m = function() {
				b.loading().hide(), $.post("index.php?i=" + a.sysinfo.uniacid + "&j=" + a.sysinfo.acid + "&c=entry&m=core&do=pay", {
					...
					if(a = $.parseJSON(a), b.loading().hide(), a.message.errno) {
						var d = {
							errno: a.message.errno,
							message: a.message.message
						};
					...
					location.href = a.message.message
				})
			};
		return b.loading().show(), c.enableMethod && c.enableMethod.length > 1 ? $.post("index.php?i=" + a.sysinfo.uniacid + "&j=" + a.sysinfo.acid + "&c=entry&m=core&do=paymethod", {
			module: c.module,
			tid: c.orderTid,
			title: c.orderTitle,
			fee: c.orderFee
		}, function(a) {
			return b.loading().hide(), g.html(a), i.setAttribute("style", ""), $(document.body).append(g), $(document.body).append(i), h(!0), g.find(".js-switch-modal").click(function() {
				j("main")
			}), g.find(".js-switch-pay").click(function() {
				j("pay")
			}), g.find(".js-switch-pay-close").click(function() {
				g.remove(), $(i).remove(), document.body.setAttribute("style", "")
			}), g.find(".js-order-title").html(c.orderTitle), g.find(".js-pay-fee").html(c.orderFee), g.find(".js-switch-pay-modal li").size() > 0 ? void(c.enableMethod && c.enableMethod.length > 0 ? g.find(".js-switch-pay-modal li").each(function() {
				-1 == $.inArray($(this).data("method"), c.enableMethod) && $(this).remove()
			}) : g.find(".js-switch-pay-modal li").each(function() {
				c.enableMethod.push($(this).data("method"))
			})) : (b.toast("暂无有效支付方式"), g.remove(), $(i).remove(), document.body.setAttribute("style", ""), !1)
		}) : "wechat" == c.payMethod ? k() : "alipay" == c.payMethod ? l() : m(), !0
